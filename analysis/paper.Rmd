...
For the calculation of bias, we used the formula provided in the original paper. 
...
```{r, ori-fig-4, fig.cap="Original figure 4", echo=FALSE}
knitr::include_graphics("figure4-ori.png")
```

```{r, ori-fig-5, fig.cap="Original figure 5", echo=FALSE}
knitr::include_graphics("figure5-ori.png")
```

The figure 4 (Fig. \@ref(fig:ori-fig-4)) and 5 (Fig. \@ref(fig:ori-fig-5)) in the original article were created by applying the best-performing model as well as an independent model (REF) to empirical data from 2010 DHS for Malawi and Tanzania respectively to estimate U5M of different age groups of population. By comparing differences in U5M estimates of unadjusted data and adjusted data by two models, the authors suggested that the new model was able to correct the bias from indirect estimates. 
...
```{r, rep-fig-4, fig.cap="Replication of figure 4 with new simulation data", echo=FALSE, fig.align='center'}
...
```{r, rep-fig-5, fig.cap="Replication of figure 5 with new simulation data", echo=FALSE, fig.align='center'}
...
We revised some parts of the regression model and found inconsistent variable names during the debugging process for the reproduction of the tables and figures to be sucessful. In the following section of the original regression code, we changed the parameter in the bootstrapping model from 1:500 to 1:5 to make it run faster. We also caught the inconsistency in the variable  “lambda.min” where the parameter should be “lambda” from the model. 
...
# Conclusion
Our goal of this project is to replicate a published research project and allow readers to produce identical figures to those in the target paper. We were able to accomplish this goal by replicating the key figures in the original paper using R and publishing well-structured source code along with data source in a licensed Github repository. This paper.md R Markdown file will be used as the main entry point of our project and is fully executable. However, we intentionally set a couple code chunks with evaluation as False since simulating those data sources does not fit within the timeframe for any group who is trying to reproduce them. Instead, we created .RData file as a battery-included solution for future reproducers.

We leveraged some useful Reproducibility techniques introduced in the class such as Tidyverse libraries and R Markdown syntax. The in-file executable R chunks come extremely handy during our reproducible process.

There are, however, challenges throughout our explorations. The first major issue we encountered was the lengthy source data generation step which required extensive research and deep dive into source code. Given the nature of the HIV/AIDS estimation research, the data samples are required to be large enough and comprehensive which should cover across multiple geological locations and a full range of populations. Therefore, the original authors designed the raw data to be generated on-the-go rather than archived in open source. Our team underestimated the challenge of reproducing all sources by the time we picked this article. To tackle this, all team members gathered together and agreed to make tradeoffs - reducing population size during simulation. The decision was made in favor of the balance between quality of our research and a timely deliverable. We identified this as an area to improve (focus on verification of data source before replication) during our retrospective of this project. Another challenge was using R in an unfamiliar field of research. Given none of us had previous domain knowledge regarding HIV/AIDS research, we spent a lot of time understanding the terminologies while exploring the rationale behind the R code used in the original paper. We had multiple issues trying to reproduce a specific failure in R from multiple environments. For example, we still couldn’t fully understand some of the issues during “knit” failures which had been resolved by cleaning up cache in R Studio. We strongly believe that using Docker or a shared development environment could dramatically improve our developer experience where all bugs could be reproduced easily no matter where the Docker image is running.

In general, we gained hands-on experience from reproducing an entire research project. After rounds of reviews and merging conflicts, we developed our own Github usage convention and code styles. We would like to carry these invaluable skills to our future Data Science journey.
